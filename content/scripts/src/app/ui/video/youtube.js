	...
	function ( $ ) {
		...
		return {
			...
			loadVideo: function ( $pane /* jQuery object */, $video /* jQuery object */ ) {
				...
				var width = $pane.attr('data-video-width') ? $pane.attr('data-video-width') : $pane.height();
				var playOnLoad = $video.attr( 'data-video-playonload' ) === 'true' ? 1 : 0;
			...
			_stopVideoEvent: function( data /*Publish Event object*/ ) {
				...
				$('#js-rotator').each(function() {
					$(this).find('.js-video-pane').each(function() {
						var $thisPane = $(this);
						if($thisPane.attr('data-video-id') === videoid) {
			...
			_prepVideo: function( $elem /* jQuery object */ ) {var $pane = this._findPane( $elem );
